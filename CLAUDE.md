...
- `src/lib/supabase/admin.ts` — service-role, **bypasses RLS**; only `POST /api/send-reminder` uses it (it must read every user's tasks)
...
**Notifications** (`POST /api/send-reminder`, called by Vercel Cron):
...
- **Vercel Hobby allows only daily crons.** A schedule more frequent than daily (e.g. `0 * * * *`) makes Vercel reject *every* deployment, silently freezing production. `vercel.json` must stay at `0 6 * * *`. True per-timezone/per-task delivery needs Vercel Pro or an external scheduler (Supabase pg_cron) hitting `/api/send-reminder`.
...
DB: run `supabase/migrations/00{1..5}_*.sql` in order in the Supabase SQL Editor. 001 tasks + RLS, 002 scheduling + `jira_issue_key`, 003 RLS-authenticated, 004 recurrence, 005 `user_prefs` + reminder columns. Supabase Auth → Redirect URLs must include `<origin>/auth/callback`.
